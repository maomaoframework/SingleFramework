...
public class JdbcDAO extends JdbcDaoSupport {
	...
	// public String getQuery(String sQueryKey) {
	// return PropertyResourceHelper.getQuery(sQueryKey);
	// }
	...
	public static String Page(String ctsql, JdbcDAO dao) {
		PageInfo pageInfo = (PageInfo) ParameterWrapper.getWrapper().get("com.cloudcard.PagePolit");
		int num = 0;
		...
		if (num < 15) {
			...
			pageWhere = " limit " + (pageInfo.getCurrentPageIndex() - 1) * 20 + "," + 15;
		}
		return pageWhere;
	}
	...
	public PageInfo getPageInfo() {
		try {
			return (PageInfo) ParameterWrapper.getWrapper().get("com.huxg.taglib.PageSplitTag");
		} catch (Exception e) {
			return null;
		}
	}
